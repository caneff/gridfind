...
(between), `type 404` (region-sum), `type 405` (sequence), `type 406`
(grouped), and `type 407` (lockout) — eight rows of the nine-relation family
...
renban, palindrome, between, sequence, and lockout state no extra param), a `disabled`
...
def test_sequence_block_decodes_to_an_ordered_path_constraint() -> None:
    payload = constraint_link({"type": 405, "lines": [[0, 1, 2]]})
    ...
    assert (
        Constraint(
            "line",
            params={"relation": "sequence", "path": ["R1C1", "R1C2", "R1C3"]},
        )
        in puzzle.constraints
    )


def test_multiple_sequence_paths_each_decode_to_their_own_constraint() -> None:
    payload = constraint_link({"type": 405, "lines": [[0, 1, 2], [9, 18, 27]]})
    ...
    assert (
        Constraint(
            "line",
            params={"relation": "sequence", "path": ["R2C1", "R3C1", "R4C1"]},
        )
        in puzzle.constraints
    )
...
@pytest.mark.parametrize(
    "block",
    [
        pytest.param({"type": 405, "lines": [[0, 1]], "disabled": True}, id="disabled"),
        pytest.param({"type": 405, "lines": []}, id="empty-lines"),
    ],
)
def test_disabled_or_empty_sequence_block_decodes_to_nothing_quietly(
    block: dict[str, object],
) -> None:
    payload = constraint_link(block)
...
def test_sequence_style_is_ignored() -> None:
    payload = constraint_link(
        {"type": 405, "lines": [[0, 1]], "style": {"color": "#000000"}}
    )

    puzzle, _ = link_to_puzzle(payload)

    assert (
        Constraint("line", params={"relation": "sequence", "path": ["R1C1", "R1C2"]})
        in puzzle.constraints
    )
