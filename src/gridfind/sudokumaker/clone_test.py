"""Decode behaviour of the `type 302` clone blocks: each group under
`input.groups` decodes to one `clone` `Constraint` carrying that group's cell
addresses, a disabled block decodes to nothing quietly, and an empty or
singleton group (no pair to equate) contributes no constraint and no warning.
"""
...
def _clone(*groups: list[int]) -> dict[str, object]:
    return {"type": 302, "input": {"groups": [{"cells": g} for g in groups]}}
