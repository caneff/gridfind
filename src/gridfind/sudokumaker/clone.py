...
`{groups: [[0, 14], [1, 15]]}`) — decoded one `clone` `Constraint` per
group: within each group the cells hold equal digit sets, each group
standing alone with no relationship to any other group in the same block
(the `Clone` layer holds the group's own cells to an equal digit set,
ADR-0019 dec 4).
