...
# constraint rather than a registry wire type of its own, and white-/
# black-kropki-negative each share their wire type (200/201) with their own
# plain positive-only decode (told apart by whether a decoded `pair-difference`/
# `pair-ratio` carries `negate`), so each type's own found/broke pair doesn't
# also prove its negative rule.
_EXPLICIT_VARIANTS = (
    "classic",
    "jigsaw",
    "schrodinger",
