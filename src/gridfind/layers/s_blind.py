...
`OffsetAdjacency` and `NumberedRooms` each read a cell through its single
content slot, which has no defined meaning once `schrodinger` widens every
cell to two. `build_stack` calls `refuse_s_blind_over_widening` to reject
the pair up front and name the offending layer, rather than let the
s-blind layer read past a widened cell's first slot alone deep in the
solve. Issue #523 tracks retiring the `s_blind` flag itself once a
widening-aware read exists for both layers; this direct check names the
two concrete layers rather than scanning the stack for the property.
"""
...
from gridfind.layers.offset_adjacency import OffsetAdjacency

_S_BLIND_LAYER_TYPES: tuple[type, ...] = (OffsetAdjacency, NumberedRooms)
