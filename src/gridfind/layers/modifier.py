...
per column, and per box, and the modified cells hold each digit once (a
...
constraint). No value change lives here — placement only. A future modifier
type (a doubler) reuses this layer and supplies only its own value fold;
nothing here is doubler-specific.
...
class ModifierPlacement:
    ...
    def _emit_transversal(
        ...
        "this cell is a modifier holding this digit"; exactly one cell must,
        binding every digit to exactly one modifier the way `emit_house`
        binds every digit to exactly one house slot.
        """
