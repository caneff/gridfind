...
cells and emits no rules of its own (decisions 7, 14). The core
holds zero geometry — `cell_address` travels with the layer that supplies it,
so a future hex board stays clean. Size and the digit values
come from `engine.board`, the puzzle's own board shape — no
