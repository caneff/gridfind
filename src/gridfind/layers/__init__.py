...
layer/preset/alias registries — lives in `layers/door.py`. Everything
beyond `__all__` is implementation detail: used in-tree, not part of the
committed surface, and free to change. Tests reach the layer classes and
`UnknownLayerError` through their own submodules (e.g.
`gridfind.layers.regions`, `gridfind.layers.door`).
