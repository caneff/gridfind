...
from gridfind.puzzle import Board, Given, Puzzle, Variant
from gridfind.verdict import verdict

BOARD = Board(size=9)


def _pair_sum(cells: tuple[str, str], total: int) -> Variant:
    return Variant(type="pair-sum", params={"cells": list(cells), "sum": total})


def _clue(kind: str, cells: tuple[str, str]) -> Variant:
    """An X or V sugar clue — names its pair, leaves the sum to the sugar."""
    return Variant(type=kind, params={"cells": list(cells)})


def test_a_satisfiable_pair_resolves_found() -> None:
    puzzle = Puzzle(board=BOARD, variants=(_pair_sum(("R1C1", "R1C2"), 5),))
...
def test_a_pair_that_cannot_meet_its_sum_resolves_broke() -> None:
    ...
    puzzle = Puzzle(
        board=BOARD,
        variants=(_pair_sum(("R1C1", "R1C2"), 5),),
...
def test_two_clues_each_constrain_their_own_pair_independently() -> None:
    ...
    puzzle = Puzzle(
        board=BOARD,
        variants=(_pair_sum(("R1C1", "R1C2"), 5), _pair_sum(("R3C3", "R3C4"), 10)),
