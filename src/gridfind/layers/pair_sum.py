...
stateless `pair-sum` instance pulls every such record via `records_of` and
emits one sum-rule per clue, so a puzzle's many clues resolve to this single
layer (issue #65). The XV variant is spelled on top of it as sugar: an X clue
is a pair-sum of 10, a V clue a pair-sum of 5 (expanded in `layers/__init__`).

Deliberately narrow (issue #66): the record names both cells outright — a
...
class PairSum:
    ...
    def emit(self, engine: Engine) -> None:
        for clue in engine.records_of(self.name):
