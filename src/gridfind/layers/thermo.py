"""The `thermo` layer: gridfind's first line type (spec #251, issue #253).

A thermo clue names an **ordered** path of cells, bulb first, and holds each
consecutive pair to a directed inequality. This slice builds the normal
(strict) flavor only: `a < b` for every edge. A two-cell path is a single
directed edge — the degenerate inequality case falls out for free, no
separate primitive needed (spec #251's "inequality is the degenerate
length-2 thermo").
...
but is not read here — non-strict (`≤`) support is issue #254's delta on
this same layer.
"""
