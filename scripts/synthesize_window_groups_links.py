...
Two groupings prove the one rule covers both SudokuMaker presets: a low/high
split (global entropy's two-group shape) on a 4x4 board, and the three mod-3
residue classes (global mod) on a 9x9 board.
...
the one under test — so each fixture's completion is chosen against the whole
board, never hand-picked to look right at one spot. A `found` completion
satisfies the rule at every window; a `broke` completion satisfies it at every
window *except* the tested one, so the verdict rests on the tested window
alone rather than on some other window the givens already spoil. Both 9x9
grids are CP-SAT witnesses of exactly that search, not hand constructions.
"""
...
# A real, valid 9x9 completion whose R4C4/R4C5/R5C4/R5C5 window forces
# 3, 8, 5, 6 — residues 0, 2, 2, 0 mod 3: the mod-1 class ({1,4,7}) never
# appears. Every *other* 2x2 window of the board does hold all three classes,
# so the tested window is the board's sole violation and the only reason the
# link reads broke. Found by solving `sudoku` plus window-groups(_MOD_GROUPS)
# at all 63 other windows, with the tested window required to miss a class.
...
def found_window_groups_mod_9x9() -> str:
    ...
    window to 1, 3, 2, 7: a digit from every residue class present."""
...
def broke_window_groups_mod_9x9() -> str:
    ...
    window to 3, 8, 5, 6: the mod-1 class ({1,4,7}) never appears."""
