...
`4 // 2 = 2` (ADR-0021, amended from a real 4x4 SudokuMaker link). R1C1 and
R3C3 are the bulbs, given 1 and 3 — a gap of exactly 2, clearing the
threshold with no room to spare — leaving 4 as the only digit strictly
outside the (1, 3) interval. `found-lockout-4x4` leaves R2C2 to the solver,
satisfied only by 4; `broke-lockout-4x4` gives R2C2 equal to R1C1's own bulb
value (1), inside the interval, a direct contradiction since all three cells
are given (and, since R1C1 and R2C2 share a box on this tiling, doubly so).
"""
...
def found_lockout_4x4() -> str:
    """4x4 lockout (bulbs 1 and 3, threshold `4 // 2 = 2`), `found` — the
    bulb gap (2) exactly clears the threshold, and the interior cell is left
    to the solver, satisfied only by 4 (the one digit strictly outside the
    (1, 3) bulb interval)."""
    return _link(r1c1=1, r3c3=3, r2c2=None)


def broke_lockout_4x4() -> str:
    """4x4 lockout (bulbs 1 and 3), `broke` — the interior cell is given 1,
    equal to a bulb and so inside the (1, 3) interval, not strictly outside
    it; unsatisfiable no matter how the rest of the board is filled, since
    all three cells are given."""
    return _link(r1c1=1, r3c3=3, r2c2=1)
