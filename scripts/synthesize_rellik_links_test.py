...
on-demand `links_test` e2e suite): each link decodes to the expected
`cage` + `rellik-cage` pair over the cage's own cells and forbidden total.
The drift guard that the committed file matches its synthesizer byte for
byte lives in `corpus_drift_test.py`, auto-discovered over every synthesizer.
"""
