...
- **Givens-on-the-clue rule** (spec #723) — no given may occupy a cell referenced by the constraint a found/broke link exists to test (a line's path, a cage's cells, a quad's four cells, a clone group, a pair's two cells, an indexer cell): a given sitting on the clue cell makes the verdict trivial regardless of the rule, so it proves nothing about the constraint under test. Row/column/box uniqueness and the region map are not "the constraint under test" and are exempt from this rule. `just givens-on-clue-check` (`scripts/audit_givens_on_clue.py`) reports every hit across the real corpus, honoring the script's own by-stem `EXEMPTIONS` list — report mode only, not yet a `check` dependency.
- **Eyeball eval links** ("show me the eval page") → hand the human `! just eval-links --changed` (a branch's new links) or `! just eval-links --all` (the whole corpus); no flag shows only links not yet approved. The recipe detaches the server itself (log: `/tmp/eval-links.log`) and the page opens in the browser, so give the plain command — never add `&`, a redirect, or a `cat` wrapper. `just eval-links-stop` kills it. **Never run it yourself**: the harness reaps the server when the tool call ends and the page dies.
